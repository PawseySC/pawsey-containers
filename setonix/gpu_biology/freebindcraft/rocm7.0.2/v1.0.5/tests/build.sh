#!/bin/bash -l
#
# Build the FreeBindCraft v1.0.5 (ROCm 7.0.2) container with podman, convert it
# to a Singularity image and run quick checks inside it (no GPU needed).
...
TOOL=freebindcraft
TAG=v1.0.5-rocm7.0.2
EXPECTED_TITLE=freebindcraft_v1.0.5_rocm7.0.2_ubuntu24.04

# Commands run inside the new image; each must succeed.
SMOKE_CHECKS=(
  "python -c 'import colabdesign, jax, openmm, pdbfixer'"
  "cd /app/FreeBindCraft && python bindcraft.py --help | grep -q no-pyrosetta"
  "test -x /app/FreeBindCraft/functions/dssp && test -x /app/FreeBindCraft/functions/DAlphaBall.gcc"
)
