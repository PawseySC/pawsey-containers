#!/bin/bash -l
#
# Build the Boltz-2 v2.2.1 (ROCm 6.4.1) container with podman, convert it to a
...
TOOL=boltz2
TAG=v2.2.1-rocm6.4.1
EXPECTED_TITLE=boltz2_v2.2.1_rocm6.4.1_ubuntu24.04

# Commands run inside the new image; each must succeed.
SMOKE_CHECKS=(
  "python3 -c 'import boltz, torch; assert torch.version.hip, \"PyTorch is not a ROCm build\"'"
  "boltz predict --help | grep -q no_kernels"
)
