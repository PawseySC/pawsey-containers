#!/bin/bash -l
#
# Build the OpenFold3 v0.4.5 (ROCm 7.2.3) container with podman, convert it to
...
TOOL=openfold3
TAG=v0.4.5-rocm7.2.3
EXPECTED_TITLE=openfold3_v0.4.5_rocm7.2.3_ubuntu24.04

# Commands run inside the new image; each must succeed.
SMOKE_CHECKS=(
  "python -c 'import openfold3, torch; assert torch.version.hip, \"PyTorch is not a ROCm build\"'"
  "python -c 'import importlib.metadata as m; v = m.version(\"openfold3\"); print(v); assert v == \"0.4.5\"'"
  "run_openfold predict --help | grep -q query-json"
)
