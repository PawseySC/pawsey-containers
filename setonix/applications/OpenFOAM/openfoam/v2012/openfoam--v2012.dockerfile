...
# The package selection below is preserved from the original v2012 recipe
...
# GCC 9 was the default compiler family in Ubuntu 20.04 and used to work fine for OpenFOAM v2012.
...
# OpenFOAM v2012 includes ADIOS2-2.4.6, whose Python bindings are incompatible with Python 3.12.
...
RUN test -f ${OF_ADIOS2_CONFIG_FILE} \
 ...
 && grep -q '^adios2_version=ADIOS2-2\.6\.0$' ${OF_ADIOS2_CONFIG_FILE} \
 && sed -i 's/^adios2_version=ADIOS2-2\.6\.0$/adios2_version=none/' ${OF_ADIOS2_CONFIG_FILE} \
...
# IMPORTANT: OpenFOAM v2012 Catalyst requires ParaView's PythonCatalyst component.
