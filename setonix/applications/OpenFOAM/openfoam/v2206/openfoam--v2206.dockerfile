...
# IMPORTANT: Global arguments marked with USER_BUILD_ARG can be overridden with `--build-arg <Name>=<Value>` using the building script.
...
RUN DEBIAN_FRONTEND=noninteractive apt-get update -qq \
 && apt-get --no-install-recommends --no-install-suggests --yes install \
    build-essential \
    # Installing the GCC version selected for compatibility with OpenFOAM v2206 on Ubuntu 24.04:
