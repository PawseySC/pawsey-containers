...
#
# The launcher:
#   1. Resolves the repository and shared GPU-MPI test directories.
...
# Run from anywhere:
#
#   ./setonix/mpi/rocm-mpich-base/testing/run_tests.sh
#
# Configuration values can be overridden for one invocation. Examples:
#
#   PARTITION=gpu ./setonix/mpi/rocm-mpich-base/testing/run_tests.sh
#   RESERVATION=PAWSEY_XXX_TEST ./setonix/mpi/rocm-mpich-base/testing/run_tests.sh
#   EXECUTION_MODE=concurrent ./setonix/mpi/rocm-mpich-base/testing/run_tests.sh
#   SINGULARITY_IMAGE=/path/to/image.sif ./setonix/mpi/rocm-mpich-base/testing/run_tests.sh
#
...
# The original script has been fully reviewed by Alexis Espinosa at Pawsey Supercomputing Centre.
...
REPO_ROCM_MPICH_BASE_DIR="$(cd "${TESTING_DIR}/.." && pwd)"
REPO_MPI_DIR="$(cd "${REPO_ROCM_MPICH_BASE_DIR}/.." && pwd)"
...
ROCM_VERSION="${ROCM_VERSION:-7.0.1}"
MPICH_VERSION="${MPICH_VERSION:-4.2.2}"
OS_VERSION="${OS_VERSION:-24.04}"
IMAGE_NAME="rocm-mpich-base"
IMAGE_TAG="rocm${ROCM_VERSION}-mpich${MPICH_VERSION}-ubuntu${OS_VERSION}"
...
run_slurm_test() {
    ...
    if ! [[ "${gpus_per_node}" =~ ^[1-9][0-9]*$ ]]; then
        echo "ERROR: Fpus per node must be a positive integer: ${gpus_per_node}" >&2
        FAILED=1
        return
    fi
    ...
    echo "Slurm error file: ${OUTPUT_DIR}/slurm-${test_name}-${job_id}.out"
}


# Run all tests:
# Use: run_slurm_test <test_script> <number_of_nodes> <tasks_per_node>
run_slurm_test "${SHARED_TESTS_DIR}/test_06_osu-gpu.slurm.sh" 2 4 4
run_slurm_test "${SHARED_TESTS_DIR}/test_07_basic-gpu-mpi-comm.slurm.sh" 2 4 4
run_slurm_test "${SHARED_TESTS_DIR}/test_08_gpu-mpi-comm.slurm.sh" 2 4 4
run_slurm_test "${SHARED_TESTS_DIR}/test_10_gpu-mpi-io.slurm.sh" 2 4 4
...
for test_name in \
    "test_06_osu-gpu" \
    "test_08_gpu-mpi-comm" \
    "test_07_basic-gpu-mpi-comm" \
    "test_10_gpu-mpi-io"
do
    marker_pass="${OUTPUT_DIR}/${test_name}.PASS"
    marker_fail="${OUTPUT_DIR}/${test_name}.FAIL"
