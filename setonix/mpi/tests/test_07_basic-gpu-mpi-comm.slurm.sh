...
# - test_gpu_mpi_comm: GPU-MPI communication tests.
...
if ! grep -Fq "running GPU_GPU_copy test" "${fileOutRun}"; then
    fail "test_gpu-mpi_comm output missing gpu-gpu copy test. See: ${fileOutRun}"
fi

if ! grep -Fq "running GPU_bandwidth_sendrecv test" "${fileOutRun}"; then
    fail "test_gpu-mpi_comm output missing sendrecv test. See: ${fileOutRun}"
fi

if ! grep -Fq "running GPU_async_sendrecv test" "${fileOutRun}"; then
    fail "test_gpu-mpi_comm output missing async sendrecv test. See: ${fileOutRun}"
fi

if ! grep -Fq "running GPU_allgather test" "${fileOutRun}"; then
    fail "test_gpu-mpi_comm output missing allreduce test. See: ${fileOutRun}"
fi
