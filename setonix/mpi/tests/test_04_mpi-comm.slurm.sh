...
# - test_mpi_comm: MPI communication tests.
...
# test_mpi_comm configuration:
# -s 0.001: maximum message size in GB.
# -i 1    : one iteration per communication test.
# -d 1    : harmless long-delay value; long-delay test is disabled by -D 0.
# -j 1    : short memory-sampling delay used between repeated tests.
# -m 1000 : harmless long-delay message size; long-delay test is disabled by -D 0.
# -r 0    : root task for root-based tests, including sendrecv_singlerank.
# -D 0    : disables long-delay send/receive test.
# -C 0    : disables correct-values send/receive test.
# -G 0    : disables gather test.
# -S 0    : disables scatter test.
# -R 1    : enables allreduce test.
# -B 1    : enables Bcast test.
# -P 1    : enables all-to-all-style sendrecv test.
# -p 1    : enables root-to-single-rank sendrecv test.
# -M 0    : disables memory-footprint mode; avoids intentional allocation sleeps
#           and ensures the run focuses on MPI communication/reporting.
# -N 0    : keeps real MPI communication enabled; do not use NOMPI memory mode.
#
# Not set:
# -o      : only used by the long-delay test, which is disabled with -D 0.
MPI_COMM_ARGS="${MPI_COMM_ARGS:--s 0.001 -i 1 -d 1 -j 1 -m 1000 -r 0 -D 0 -C 0 -G 0 -S 0 -R 1 -B 1 -P 1 -p 1 -M 0 -N 0}"
...
#--- Output files
fileOutBinaryCheck="${OUTPUT_DIR}/res_${TEST_NAME}.test_mpi_comm_binaries.out"
fileOutLinkage="${OUTPUT_DIR}/res_${TEST_NAME}.test_mpi_comm_linkage.out"
fileOutRun="${OUTPUT_DIR}/res_${TEST_NAME}.test_mpi_comm_run.out"
...
#--- Linkage check
echo
echo "=== test_mpi_comm linkage check ==="

if ! singularity exec "${SINGULARITY_IMAGE}" ldd "${MPI_COMM_EXE}" \
    | grep -E 'profile_util|mpi|fabric|cxi|pmi|pmix|pals|xpmem' \
...
#--- Runtime test
echo
echo "=== test_mpi_comm runtime test ==="
...
if ! srun -N "${SLURM_JOB_NUM_NODES}" \
    ...
    fail "test_mpi_comm run failed. See: ${fileOutRun}"
fi
...
#--- Runtime validation
echo
echo "=== Validate test_mpi_comm output ==="
...
echo
echo "=== Validate test_mpi_comm reporting/performance ==="
...
{
    echo "METRIC test_mpi_comm_elapsed_time_sec ${ELAPSED_TIME_SEC}"
    echo "METRIC test_mpi_comm_timing_lines ${TIMING_LINE_COUNT}"
    echo "METRIC test_mpi_comm_tag_world_timing_lines ${TAG_WORLD_TIMING_COUNT}"
    echo "METRIC test_mpi_comm_memory_report_lines ${MEMORY_REPORT_COUNT}"
    echo "METRIC test_mpi_comm_allreduce_tag_world_max_average_usec ${ALLREDUCE_TAG_WORLD_MAX_AVE_USEC}"
    echo "METRIC test_mpi_comm_sendrecv_tag_world_max_average_usec ${SENDRECV_TAG_WORLD_MAX_AVE_USEC}"
...
echo "Observed test_mpi_comm elapsed time               : ${ELAPSED_TIME_SEC} sec"
...
if compare_float_gt "${ELAPSED_TIME_SEC}" "${FAIL_TIME_SEC}"; then
    fail "test_mpi_comm elapsed time ${ELAPSED_TIME_SEC} sec exceeded failure threshold ${FAIL_TIME_SEC} sec. See: ${fileOutPerf}"
fi

if compare_float_gt "${ELAPSED_TIME_SEC}" "${WARN_TIME_SEC}"; then
    warn "test_mpi_comm elapsed time ${ELAPSED_TIME_SEC} sec exceeded warning threshold ${WARN_TIME_SEC} sec. See: ${fileOutPerf}"
fi
...
echo
echo "test_mpi_comm validation passed"
